...
#pragma mark - ZspIndexPath
@implementation ZspIndexPath
...
+ (instancetype)indexPathWithColumn:(NSInteger)column row:(NSInteger)row {
    ZspIndexPath *path = [[self alloc] initWithColumn:column row:row];
    return path;
}
...
#pragma mark - ZspMenu
@interface ZspMenu () <UITableViewDataSource, UITableViewDelegate>
{
    struct {
...
- (void)setDataSource:(id<ZspMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numberOfColumn; i++) {
        ...
        if (!_isClickHaveItemValid && [_dataSource menu:self numberOfItemsInRow:0 inColumn:i] > 0 && _dataSourceFlag.numberOfItemsInRow) {
            titleString = [_dataSource menu:self titleForItemsInRowAtIndexPath:[ZspIndexPath indexPathWithColumn:i row:0 item:0]];
        }else {
            titleString = [_dataSource menu:self titleForRowAtIndexPath:[ZspIndexPath indexPathWithColumn:i row:0]];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == _leftTableView) {
        if (_dataSourceFlag.titleForRowsAtIndexPath) {
            cell.textLabel.text = [_dataSource menu:self titleForRowAtIndexPath:[ZspIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
            if (_dataSourceFlag.imageNameForRowAtIndexPath) {
                NSString *imgName = [_dataSource menu:self imageNameForRowAtIndexPath:[ZspIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
            ...
            if (_dataSourceFlag.detailTextForRowAtIndexPath) {
                cell.detailTextLabel.text = [_dataSource menu:self detailTextForRowAtIndexPath:[ZspIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
            }else {
        ...
        if (_dataSourceFlag.titleForItemInRowAtIndexPath) {
            ...
            cell.textLabel.text = [_dataSource menu:self titleForItemsInRowAtIndexPath:[ZspIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
            if (_dataSourceFlag.imageNameForItemInRowAtIndexPath) {
                NSString *imgName = [_dataSource menu:self imageForItemsInRowAtIndexPath:[ZspIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
                if (imgName && imgName.length > 0) {
                    cell.imageView.image = [UIImage imageNamed:imgName];
                }else {
                    cell.imageView.image = nil;
            ...
            if (_dataSourceFlag.detailTextForItemInRowAtIndexPath) {
                cell.detailTextLabel.text = [_dataSource menu:self detailTextForItemsInRowAtIndexPath:[ZspIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedRow item:indexPath.row]];
            }else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == _leftTableView) {
        ...
        if (isClickHaveItemValid && _delegate &&[_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            [_delegate menu:self didSelectRowAtIndexPath:[ZspIndexPath indexPathWithColumn:_currentSelectedColumn row:indexPath.row]];
        }
    }else {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            [_delegate menu:self didSelectRowAtIndexPath:[ZspIndexPath indexPathWithColumn:_currentSelectedColumn row:[_currentSelectedRows[_currentSelectedColumn] integerValue] item:indexPath.item]];
        }
    }
}
...
- (void)selectIndexPath:(ZspIndexPath *)indexPath {
    ...
    if (indexPath.item < 0 ) {
        ...
        if (!_isClickHaveItemValid && [_dataSource menu:self numberOfItemsInRow:indexPath.row inColumn:indexPath.column] > 0){
            title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[ZspIndexPath indexPathWithColumn:indexPath.column row:indexPath.row]];
                [_delegate menu:self didSelectRowAtIndexPath:[ZspIndexPath indexPathWithColumn:indexPath.column row:indexPath.row item:0]];
        }else {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:[ZspIndexPath indexPathWithColumn:indexPath.column row:indexPath.row]];
...
- (BOOL)setMenuWithSelectedRow:(NSInteger)row {
    ...
    if (_dataSourceFlag.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:row inColumn:_currentSelectedColumn] > 0) {
        if (_isClickHaveItemValid) {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:[ZspIndexPath indexPathWithColumn:_currentSelectedColumn row:row]];
...
- (void)setMenuWithSelectedItem:(NSInteger)item {
    ...
    title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[ZspIndexPath indexPathWithColumn:_currentSelectedColumn row:currentSelectedMenudRow item:item]];
    [self animateIndicator:_indicators[_currentSelectedColumn] background:_backGroundView tableView:_leftTableView title:title reverse:NO complecte:^{
        _isShow = NO;
    }];
}
